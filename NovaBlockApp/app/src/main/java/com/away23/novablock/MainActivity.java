...
public class MainActivity extends Activity {
    ...
    private void checkForUpdates() {
        new Thread(() -> {
            ...
            try {
                ...
                if (latestCode > getInstalledVersionCode() && !updateUrl.isEmpty()) mainHandler.post(() -> showUpdateDialog(latestName, changelog, updateUrl));
    ...
    private void showUpdateDialog(String versionName, String changelog, String updateUrl) {
        ...
        AlertDialog dialog = new AlertDialog.Builder(this)
                ...
                .setNegativeButton("ALTE VERSION SPIELEN", (d, which) -> d.dismiss()).create();
        dialog.setOnShowListener(d -> { dialog.getButton(AlertDialog.BUTTON_POSITIVE).setAllCaps(false); dialog.getButton(AlertDialog.BUTTON_NEGATIVE).setAllCaps(false); });
        dialog.show();
    }

    private void openUpdate(String updateUrl) {
        try { Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(updateUrl)); intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK); startActivity(intent); }
        catch (Exception e) { Toast.makeText(this, "Update konnte nicht geöffnet werden.", Toast.LENGTH_LONG).show(); }
    }
